...
**类内顺序**写在同一文件的 `order` 里；行序与每行的「排序依据」由宿主常量
`CATEGORIES`（`lib/index.js`）给出 —— 面板只负责显示，不自己排序。

| 分类 | 排序依据 | 家族 |
| --- | --- | --- |
| `朋克美学` | 按光谱轴 | 七个朋克家族（可疑的未来 → 可向往的未来） |
| `千禧美学` | 按血缘与年代 | 孟菲斯 → 网页 1.0 → Y2K → Frutiger Aero → McBling → 中文千禧 |
| `星际争霸` | 按种族 | 人族 / 虫族 / 神族（不在时间轴上） |

**排序依据会换行显示在分类名下方**：只写分类名，用户看不出这一行是按什么排的；
而并排会让标签列按两者里最长的那一条撑宽，依据只是注解，不该挤占横向空间。
面板里所有分类行**共用一个网格**（行用 `display:contents` 参与外层网格），
所以标签列宽度按分类名算、各行的家族 chip 左边缘对齐。

分类漏写、写成没声明过的值、或漏了 `order`，都会在目录册里报诊断 ——
否则它会悄悄混进别的行、或落到该分类末尾，而面板上没有任何东西提示这件事。
