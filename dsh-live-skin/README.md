...
| 5b CSS 审计 | 对**每一个 variant** 逐一执行：选择器作用域（C2）、关键帧只动合成属性（C5）、`backdrop-filter` 只在小浮层（C6）、稳定锚点（C7）、零外部资产（C8）、不注入文本（C9）、不重复声明参数的 cssVar（C10）、number 参数的兜底值同型（C11）、静态可比对的对比度（C4） |
